...
if __name__ == '__main__':
    ...
    # Patch sigma so that we can create a dense array as the lookup table.
    sigma.sort()
    assert len(sigma) > 1
    if sigma[1] != (1, Unknown):
        sigma.insert(1, (1, Unknown))
    assert len(sigma) > 2
    if sigma[2] != (2, Identity):
        sigma.insert(2, (2, Identity))
    ...
    with open(args.output, 'wt', encoding='UTF-8') as output_file:
        ...
        # Add a disclaimer, so people hopefully do not modify this file.
        output('# -*- coding: UTF-8 -*-')
        output('# AUTOGENERATED FILE. DO NOT MODIFY!')
        output('# Created from', args.filename)
        output()

        # Insert libfsm verbatim.
        output('# START INCLUDE libfsm #')
        with open('libfsm.py', 'rt') as libfsm_file:
            for line in libfsm_file:
                output(line, end='')
        output('# END INCLUDE libfsm #')
        output()
